                    ...
                    <div className="flex justify-between items-center mb-6">
                        <div>
                            ...
                            <p className="mt-1 text-sm text-neutral-600">
                                Gestiona tus fuentes de ingresos recurrentes y únicos.
                            </p>
                        </div>
                        <div>
                    ...
                    <div className="bg-white rounded-lg shadow-sm">
                        {loading ? (
                            ...
                            <ul className="divide-y divide-neutral-200">
                                {incomes.map(income => (
                                    <li key={income.id} className="p-4 hover:bg-neutral-50 flex justify-between items-center">
                                        <div className="flex items-center gap-4 flex-grow">
                                            <div className={`w-10 h-10 rounded-full flex items-center justify-center ${(categoriesMap[income.categoryId] || {}).color || 'bg-neutral-500'}`}>
                                                {React.createElement(icons[(categoriesMap[income.categoryId] || {}).icon] || FaQuestionCircle, { className: 'w-5 h-5 text-white' })}
                                            </div>
                                            <div className="flex-grow">
                                                <p className="font-semibold text-neutral-800">{income.name}</p>
                                        ...
                                        <div className="flex items-center space-x-2">
                                            <button onClick={() => handleEdit(income)} className="text-sm bg-neutral-200 hover:bg-neutral-300 text-neutral-800 font-semibold py-1 px-3 rounded-lg transition-colors">
                                                Editar
                                            </button>
                                            <button onClick={() => handleDelete(income.id)} className="text-sm bg-red-500 hover:bg-red-600 text-white font-semibold py-1 px-3 rounded-lg transition-colors">
                                                Eliminar
                                            </button>
                                        </div>
                                    </li>
