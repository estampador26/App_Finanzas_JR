...
const RecurringPaymentModal = ({ isOpen, onClose, user, paymentToEdit }) => {
  ...
  const initialFormState = {
    ...
    name: '',
    paymentDay: '',
    startDate: '', // Renamed from firstPaymentDate
    endDate: '', // Optional
    // Debt specific
    initialAmount: '',
  ...
  useEffect(() => {
    if (isOpen) {
      if (isEditMode) {
        ...
        // Create a new form data object based on the payment to edit
        const newFormData = { ...initialFormState, ...paymentToEdit };

        // Handle start date (backwards compatible with 'firstPaymentDate')
        const startDateToFormat = paymentToEdit.startDate || paymentToEdit.firstPaymentDate;
        if (startDateToFormat && startDateToFormat.toDate) {
          newFormData.startDate = startDateToFormat.toDate().toISOString().split('T')[0];
        } else {
          newFormData.startDate = ''; // Ensure empty string to avoid uncontrolled component warning
        }

        // Handle end date
        if (paymentToEdit.endDate && paymentToEdit.endDate.toDate) {
          newFormData.endDate = paymentToEdit.endDate.toDate().toISOString().split('T')[0];
        } else {
          newFormData.endDate = ''; // Ensure empty string
        }
  ...
  const handleSubmit = async (e) => {
    ...
    const basePayload = {
      ...
      startDate: formData.startDate ? toTimestamp(formData.startDate) : null,
      endDate: formData.endDate ? toTimestamp(formData.endDate) : null,
    };
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {paymentType === 'debt' ? (
            ...
            <>
              ...
              {(formData.liabilityType === 'revolving' || formData.liabilityType === 'loan') && (
                <>
                  ...
                  <FormInput id="startDate" label="Fecha de Inicio del Primer Pago" type="date" value={formData.startDate} onChange={handleChange} required />
                </>
              )}
            ...
            <>
              ...
              <FormInput id="paymentDay" label="Día de Pago (1-31)" type="number" min="1" max="31" value={formData.paymentDay} onChange={handleChange} required />
              <FormInput id="startDate" label="Fecha de Inicio del Pago" type="date" value={formData.startDate} onChange={handleChange} required />
