            ...
            <div className="p-4 md:p-8">
                <div className="max-w-7xl mx-auto">
                    <Header 
                        ...
                        subtitle="Crea y administra tus categorías para ingresos y gastos."
                    >
                        ...
                        {loading ? (
                            ...
                            <ul className="divide-y divide-neutral-200">
                                {categories.map(category => (
                                    <li key={category.id} className="grid grid-cols-[1fr_auto] items-center gap-4 p-4 hover:bg-neutral-50">
                                        {/* -- Main content area -- */}
                                        <div className="flex items-center gap-4 overflow-hidden">
                                            <div className={`w-10 h-10 rounded-full flex-shrink-0 flex items-center justify-center ${category.color || 'bg-neutral-500'}`}>
                                                {React.createElement(icons[category.icon] || FaQuestionCircle, { className: 'w-5 h-5 text-white' })}
                                            </div>
                                            <div className="truncate">
                                                <p className="font-semibold text-neutral-800 truncate">{category.name}</p>
                                        ...
                                        <div className="flex items-center gap-2">
                                            <button onClick={() => handleEdit(category)} className="text-sm bg-neutral-200 hover:bg-neutral-300 text-neutral-800 font-semibold py-1 px-3 rounded-lg transition-colors flex-shrink-0">
                                                Editar
                                            </button>
                                            <button onClick={() => handleDelete(category.id)} className="text-sm bg-red-500 hover:bg-red-600 text-white font-semibold py-1 px-3 rounded-lg transition-colors flex-shrink-0">
                                                Eliminar
                                            </button>
                                        </div>
                                    </li>
